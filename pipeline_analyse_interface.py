...
class pipeline_analyse_to_db(database_connection):
    ...
    def add_unique_entry(self, table_name, attribute_name, value):
        if self.fetch_command("SELECT * FROM {0} WHERE {1} = %s".format(table_name, attribute_name), (value,)) == []:
            self.enter_command("INSERT INTO {0} ({1}) VALUES (%s)".format(table_name, attribute_name), (value,))
        return self.fetch_command("SELECT {0}_id FROM {1} WHERE {0} = %s".format(attribute_name, table_name), (value,))
